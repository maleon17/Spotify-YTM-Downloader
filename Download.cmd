...
REM --- Auto-configure yt-dlp Node.js runtime if missing ---
if not exist "%USERPROFILE%\AppData\Roaming\yt-dlp\config" (
    if exist "%LOCALAPPDATA%\hermes\node\node.exe" (
        if not exist "%USERPROFILE%\AppData\Roaming\yt-dlp" md "%USERPROFILE%\AppData\Roaming\yt-dlp"
        (
            echo --js-runtimes node:"%LOCALAPPDATA:\=/%/hermes/node/node.exe"
        ) > "%USERPROFILE%\AppData\Roaming\yt-dlp\config"
    ) else (
        if exist Settings if not exist Settings\jsRuntimeWarned.txt (
            echo NOTE: No bundled JS runtime was found for yt-dlp ^(expected at "%LOCALAPPDATA%\hermes\node\node.exe"^).
            echo YouTube may block some downloads without it. Install Node.js if downloads keep failing.
            echo.
            echo shown > Settings\jsRuntimeWarned.txt
        )
    )
)
...
:recoverCheck
if exist "%~dp0Cache" goto doRecover
if exist "%~dp0YTMusic" goto doRecover
if exist "%~dp0Redistributables\dlProgress" goto doRecover
goto recoverAfter

:doRecover
echo A previous download appears to have been interrupted (e.g. the window was closed early).
echo Recovering your workspace...
echo.
if not exist "%~dp0YTMusic" goto recoverSkipMove
set "recoverFolder=%~dp0Recovered %date:/=_% %time::=-%"
md "%recoverFolder%" >nul 2>&1
move "%~dp0YTMusic\*" "%recoverFolder%" >nul 2>&1
for /d %%a in ("%~dp0YTMusic\*") do move /y "%%~fa" "%recoverFolder%" >nul 2>&1
rd "%~dp0YTMusic" >nul 2>&1
dir /b "%recoverFolder%" 2>nul | findstr "." >nul
if errorlevel 1 (rd "%recoverFolder%" >nul 2>&1) else (echo Partially downloaded tracks were recovered to "%recoverFolder%".)
:recoverSkipMove
if exist "%~dp0Cache" rd /s /q "%~dp0Cache"
if exist "%~dp0Redistributables\dlProgress" del /q "%~dp0Redistributables\dlProgress"
if exist "%~dp0Redistributables\Track.txt" del /q "%~dp0Redistributables\Track.txt"
if exist "%~dp0Redistributables\TotalTracks.txt" del /q "%~dp0Redistributables\TotalTracks.txt"
if exist "%~dp0Redistributables\LastRun.txt" del /q "%~dp0Redistributables\LastRun.txt"
if exist "%~dp0Redistributables\progressTickerRun.txt" del /q "%~dp0Redistributables\progressTickerRun.txt"
if exist "%~dp0Redistributables\progressState.txt" del /q "%~dp0Redistributables\progressState.txt"
if exist "%~dp0Redistributables\progressChomp.txt" del /q "%~dp0Redistributables\progressChomp.txt"
echo.
echo Your original queue (URLs.txt) is untouched. Remaining tracks will resume now;
echo anything you already downloaded will be skipped automatically.
echo.
echo Press any key to continue...
pause >nul
cls
